$(function () {
    emis.CreateNamespace('studentTracingApplication');
    ...
    (function (context) {
        ...
        context.Mapping = {
            create: function (options) {
                ...
                //vm.RecommendationLetterDocument = ko.observable(dataModel);
                //vm.CitizenshipDocument = ko.observable(dataModel);

                //vm.CurrentRecordTemplateRender = function (item, data) {
                //    context.LoadSavedImage(data);
                //}
                vm.ViewDetailClick = function (item) {
                    ajaxRequest('/Student/TracingApplication/Detail', 'POST', { data: { id: item.DataCollectionId() } },
                        function (response) {
                            if (response.IsSuccess) {
                                ko.mapping.fromJS(response.Data, {}, vm.CurrentRecord);
                                context.LoadSavedImage();
                            } else {
                ...
                vm.currentRecordRenderComplete = function () {
        ...
        context.LoadSavedImage = function () {
            if (context.ViewModel.CurrentRecord.CitizenshipUserAttachmentId() > 0) {
                ajaxRequest('/FileUpload/GetFile', 'POST', { data: { id: context.ViewModel.CurrentRecord.CitizenshipUserAttachmentId() } }, function (response) {
                    if (response.IsSuccess) {
                        context.ViewModel.CurrentRecord.CitizenshipDocument.Base64Content(response.Data)
                    } else {
                        showMessage(context.Title, response.Message, 'error')
                    }
                })
            } else {
                context.ViewModel.CurrentRecord.CitizenshipDocument.Base64Content('')
            }
            if (context.ViewModel.CurrentRecord.RecommendationLetterUserAttachmentId() > 0) {
                ajaxRequest('/FileUpload/GetFile', 'POST', { data: { id: context.ViewModel.CurrentRecord.RecommendationLetterUserAttachmentId() } }, function (response) {
                    if (response.IsSuccess) {
                        context.ViewModel.CurrentRecord.RecommendationLetterDocument.Base64Content(response.Data)
                    } else {
                        showMessage(context.Title, response.Message, 'error')
                    }
                })
            } else {
                context.ViewModel.CurrentRecord.RecommendationLetterDocument.Base64Content('')
            }
        }
        ...
        context.Initialize = function (model, currentRecord) {
            if (!ko.dataFor($('#mainContent')[0])) {
                currentRecord.RecommendationLetterDocument = ko.toJS(ko.mapping.fromJS(dataModel));
                currentRecord.CitizenshipDocument = ko.toJS(ko.mapping.fromJS(dataModel));
                context.ViewModel = ko.mapping.fromJS({ Records: model, CurrentRecord: currentRecord }, context.Mapping);
    ...
    })(emis.studentTracingApplication);
});