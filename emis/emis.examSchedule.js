    ...
    (function (context) {
        ...
        context.MappingCreate = {
            create: function (options) {
                ...
                vm.RegistrationStartDate.subscribe(function (newValue) {
                    ...
                    ajaxRequest('/Lookup/ConvertAdToBs/', 'POST', { data: { date: newDate } }, function (response) {
                        if (response.IsSuccess) {
                            vm.RegistrationStartDateBS(response.Data);
                        }
                        else {
                            vm.RegistrationStartDateBS('');
                        }
                    });
                });

                vm.RegistrationEndDate.subscribe(function (newValue) {
                    var newDate = moment(newValue).isValid() ? moment(newValue).format('YYYY-MM-DD') : '';
                    ajaxRequest('/Lookup/ConvertAdToBs/', 'POST', { data: { date: newDate } }, function (response) {
                        if (response.IsSuccess) {
                            vm.RegistrationEndDateBS(response.Data);
                        }
                        else {
                            vm.RegistrationEndDateBS('');
                        }
                    });
                });
        ...
        context.ApplyCreateValidation = function () {
            $('#create-frm').validate({
                rules: {
                    AcademicYearID: {
                        required: true
                    ...
                    RegistrationEndDate: {
                        required: true,
                        greaterThan: "#RegistrationStartDate"
                    }
                },
                messages: {
                    AcademicYearID: {
                    ...
                    ProgramId: {
                        required: 'At least one Program must be selected.'
                    },
                    YearPartID: {
                        required: 'Year Part is required.'
                    },
                    ExamScheduleName: {
                        required: 'Exam Schedule Name is required.'
                    },
                    RegistrationStartDate: {
                    ...
                    RegistrationEndDate: {
                        ...
                        greaterThan: 'Registration End Date must be after Start Date.'
                    }
                }
            });
