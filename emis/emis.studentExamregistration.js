$(function () {
    emis.CreateNamespace('studentExamregistration');
    ...
    (function (context) {
        ...
        context.Mapping = {
            create: function (options) {
                ...
                vm.SubjectSelectionChanged = function ($this) {
                    console.log($this);
                }
        ...
        context.CurrentUser = function (currentUserData) {
            ko.mapping.fromJS(currentUserData, {}, context.ViewModel.ApplicationViewModel);
            context.LoadYearPart();
            //context.IsRegistrationNoVerified(true);
        }
        ...
        context.LoadYearPart = function () {
            if (context.ViewModel.ApplicationViewModel.ProgramID()) {
                var yearPartId = context.ViewModel.ApplicationViewModel.SelectedYearPartId();
                ajaxRequest('/Cascade/GetYearPartByProgram', 'POST', { data: { programId: context.ViewModel.ApplicationViewModel.ProgramID(), yearPartId: yearPartId } }, function (response) {
                    var yearParts = [];
                    if (response.IsSuccess) {
            ...
            if (context.ViewModel.ApplicationViewModel.ProgramID && context.ViewModel.ApplicationViewModel.ProgramID() > 0) {
                ...
                ajaxRequest('/Cascade/GetSubjectRegistrationFromProgramId', 'POST', { data: m }, function (response) {
                    if (response.IsSuccess) {
                        ...
                        context.ViewModel.ApplicationViewModel.PartialSubjectRegistration.AllSubjects([]);
                        context.ViewModel.ApplicationViewModel.PartialSubjectRegistration.SubjectGroups([]);
                        showMessage(context.Title, response.Message, 'error');
                    }
                });
            }
